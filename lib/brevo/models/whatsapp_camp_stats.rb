  ...
  class WhatsappCampStats
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::WhatsappCampStats` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::WhatsappCampStats`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'not_sent')
        self.not_sent = attributes[:'not_sent']
      else
        self.not_sent = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @not_sent.nil?
        invalid_properties.push('invalid value for "not_sent", not_sent cannot be nil.')
      end
    ...
    def hash
      [sent, delivered, read, unsubscribe, not_sent].hash
    end
