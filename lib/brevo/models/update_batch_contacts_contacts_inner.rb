  ...
  class UpdateBatchContactsContactsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::UpdateBatchContactsContactsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::UpdateBatchContactsContactsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'email_blacklisted')
        self.email_blacklisted = attributes[:'email_blacklisted']
      end

      if attributes.key?(:'sms_blacklisted')
        self.sms_blacklisted = attributes[:'sms_blacklisted']
      end
      ...
      if attributes.key?(:'unlink_list_ids')
        if (value = attributes[:'unlink_list_ids']).is_a?(Array)
          self.unlink_list_ids = value
        end
      end

      if attributes.key?(:'smtp_blacklist_sender')
        if (value = attributes[:'smtp_blacklist_sender']).is_a?(Array)
