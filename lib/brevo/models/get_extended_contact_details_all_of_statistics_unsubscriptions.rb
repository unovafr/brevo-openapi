...
module Brevo
  ...
  class GetExtendedContactDetailsAllOfStatisticsUnsubscriptions
    ...
    def self.openapi_types
      {
        :'user_unsubscription' => :'Array<GetExtendedContactDetailsAllOfStatisticsUnsubscriptionsUserUnsubscription>',
        :'admin_unsubscription' => :'Array<GetExtendedContactDetailsAllOfStatisticsUnsubscriptionsAdminUnsubscription>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::GetExtendedContactDetailsAllOfStatisticsUnsubscriptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::GetExtendedContactDetailsAllOfStatisticsUnsubscriptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'user_unsubscription')
        if (value = attributes[:'user_unsubscription']).is_a?(Array)
          self.user_unsubscription = value
        end
      else
        self.user_unsubscription = nil
      end

      if attributes.key?(:'admin_unsubscription')
        if (value = attributes[:'admin_unsubscription']).is_a?(Array)
          self.admin_unsubscription = value
        end
      else
        self.admin_unsubscription = nil
      end
    end
