  ...
  class CreateUpdateProducts
    ...
    def self.openapi_types
      {
        ...
        :'parent_id' => :'String',
        :'meta_info' => :'Hash<String, GetContactInfoIdentifierParameter>',
        :'deleted_at' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::CreateUpdateProducts` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::CreateUpdateProducts`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'meta_info')
        if (value = attributes[:'meta_info']).is_a?(Hash)
    ...
    def hash
      [id, name, url, image_url, sku, price, categories, parent_id, meta_info, deleted_at].hash
    end
