  ...
  class CreateEmailCampaign
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::CreateEmailCampaign` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::CreateEmailCampaign`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'unsubscription_page_id')
        self.unsubscription_page_id = attributes[:'unsubscription_page_id']
      end

      if attributes.key?(:'update_form_id')
        self.update_form_id = attributes[:'update_form_id']
      end
    end
    ...
    def split_rule=(split_rule)
      if split_rule.nil?
        fail ArgumentError, 'split_rule cannot be nil'
      end
    ...
    def winner_delay=(winner_delay)
      if winner_delay.nil?
        fail ArgumentError, 'winner_delay cannot be nil'
      end
    ...
    def increase_rate=(increase_rate)
      if increase_rate.nil?
        fail ArgumentError, 'increase_rate cannot be nil'
      end
