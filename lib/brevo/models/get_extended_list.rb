  ...
  class GetExtendedList
    ...
    def self.openapi_types
      {
        ...
        :'folder_id' => :'Integer',
        :'created_at' => :'String',
        :'campaign_stats' => :'Array<GetExtendedListAllOfCampaignStats>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::GetExtendedList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::GetExtendedList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_blacklisted')
        self.total_blacklisted = attributes[:'total_blacklisted']
      else
        self.total_blacklisted = nil
      end

      if attributes.key?(:'total_subscribers')
        self.total_subscribers = attributes[:'total_subscribers']
      else
        self.total_subscribers = nil
      end

      if attributes.key?(:'unique_subscribers')
        self.unique_subscribers = attributes[:'unique_subscribers']
      else
        self.unique_subscribers = nil
      end
      ...
      if attributes.key?(:'dynamic_list')
        self.dynamic_list = attributes[:'dynamic_list']
      end
    end
