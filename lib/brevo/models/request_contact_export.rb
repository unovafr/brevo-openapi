  ...
  class RequestContactExport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::RequestContactExport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::RequestContactExport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'export_attributes')
        if (value = attributes[:'export_attributes']).is_a?(Array)
          self.export_attributes = value
        end
      end

      if attributes.key?(:'contact_filter')
        self.contact_filter = attributes[:'contact_filter']
      end

      if attributes.key?(:'custom_contact_filter')
        self.custom_contact_filter = attributes[:'custom_contact_filter']
      end
