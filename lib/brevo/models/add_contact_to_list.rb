...
module Brevo
  class AddContactToList
    ...
    # IDs to add to a list. You can pass a maximum of 150 IDs for addition in one request. If you need to add the emails in bulk, please prefer /contacts/import api.
    attr_accessor :ids
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Brevo::AddContactToList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Brevo::AddContactToList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@emails.nil? && @emails.length > 150
        invalid_properties.push('invalid value for "emails", number of items must be less than or equal to 150.')
      end
    ...
    def emails=(emails)
      ...
      if emails.length > 150
        fail ArgumentError, 'invalid value for "emails", number of items must be less than or equal to 150.'
      end
